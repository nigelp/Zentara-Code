...
const TestComponent = () => {
	const { allowedCommands, setAllowedCommands, soundEnabled, showZentaraIgnoredFiles, setShowZentaraIgnoredFiles } =
		useExtensionState()
	...
	return (
		<div>
			...
			<div data-testid="show-zentaraignored-files">{JSON.stringify(showZentaraIgnoredFiles)}</div>
			<button data-testid="update-button" onClick={() => setAllowedCommands(["npm install", "git status"])}>
				Update Commands
			</button>
			<button data-testid="toggle-zentaraignore-button" onClick={() => setShowZentaraIgnoredFiles(!showZentaraIgnoredFiles)}>
				Update Commands
			</button>
		</div>
	)
...
describe("ExtensionStateContext", () => {
	...
	it("initializes with showZentaraIgnoredFiles set to true", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		expect(JSON.parse(screen.getByTestId("show-zentaraignored-files").textContent!)).toBe(true)
	})

	it("updates showZentaraIgnoredFiles through setShowZentaraIgnoredFiles", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		act(() => {
			screen.getByTestId("toggle-zentaraignore-button").click()
		})

		expect(JSON.parse(screen.getByTestId("show-zentaraignored-files").textContent!)).toBe(false)
	})
