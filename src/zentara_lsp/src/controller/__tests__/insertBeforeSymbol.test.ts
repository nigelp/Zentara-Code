...
import { InsertBeforeSymbolParams, InsertBeforeSymbolParamsLegacy } from '../../types'
...
describe('insertBeforeSymbol', () => {
	...
	describe('backward compatibility with legacy format', () => {
		it('should work with legacy textDocument/position parameters', async () => {
			const legacyParams: InsertBeforeSymbolParamsLegacy = {
			...
			const result = await insertBeforeSymbol(legacyParams)
	...
	describe('error handling', () => {
		...
		it('should handle exceptions gracefully', async () => {
			...
			expect(result).toEqual({
				success: false,
				content: 'Error inserting before symbol: Test error'
			})

			expect(consoleSpy).toHaveBeenCalledWith(
				'Error inserting before symbol:',
				expect.any(Error)
			)
