...
import {
	getGlobalZentaraDirectory,
	getProjectZentaraDirectoryForCwd,
	directoryExists,
	fileExists,
	readFileIfExists,
	getZentaraDirectoriesForCwd,
	loadConfiguration,
} from "../index"

describe("ZentaraConfigService", () => {
	beforeEach(() => {
		vi.clearAllMocks()
	...
	describe("getGlobalZentaraDirectory", () => {
		it("should return correct path for global .zentara directory", () => {
			const result = getGlobalZentaraDirectory()
			expect(result).toBe(path.join("/mock/home", ".zentara"))
		})

		it("should handle different home directories", () => {
			mockHomedir.mockReturnValue("/different/home")
			const result = getGlobalZentaraDirectory()
			expect(result).toBe(path.join("/different/home", ".zentara"))
		})
	})

	describe("getProjectZentaraDirectoryForCwd", () => {
		it("should return correct path for given cwd", () => {
			const cwd = "/custom/project/path"
			const result = getProjectZentaraDirectoryForCwd(cwd)
			expect(result).toBe(path.join(cwd, ".zentara"))
		})
	})
	...
	describe("getZentaraDirectoriesForCwd", () => {
		it("should return directories for given cwd", () => {
			const cwd = "/custom/project/path"

			const result = getZentaraDirectoriesForCwd(cwd)

			expect(result).toEqual([path.join("/mock/home", ".zentara"), path.join(cwd, ".zentara")])
		})
	})
	...
	describe("loadConfiguration", () => {
		...
		it("should use correct file paths", async () => {
			...
			expect(mockReadFile).toHaveBeenCalledWith(path.join("/mock/home", ".zentara", "rules/rules.md"), "utf-8")
			expect(mockReadFile).toHaveBeenCalledWith(path.join("/project/path", ".zentara", "rules/rules.md"), "utf-8")
		})
	})
})
