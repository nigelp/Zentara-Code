...
export class GCliOAuthManagerV2 {
	...
	async getProjectId(token: string): Promise<string | null> {
		try {
			console.log("🔍 Getting project ID using onboarding flow...")
			const onboardingManager = new GCliOnboardingManager()
			return await onboardingManager.setupUserAndGetProjectId(token)
		} catch (error) {
