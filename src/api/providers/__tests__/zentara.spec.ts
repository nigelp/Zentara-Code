// npx vitest run api/providers/__tests__/zentara.spec.ts

import { Anthropic } from "@anthropic-ai/sdk"
import { zentaraDefaultModelId, zentaraModels } from "@zentara-code/types"
...
vitest.mock("@zentara-code/cloud", () => ({
	CloudService: {
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key: string) => {
		if (key === "common:errors.zentara.authenticationRequired") {
			return "Authentication required for Zentara Code Cloud"
		}
		return key
	}),
}))
...
import { ZentaraHandler } from "../zentara"
import { CloudService } from "@zentara-code/cloud"

describe("ZentaraHandler", () => {
	let handler: ZentaraHandler
	...
	describe("constructor", () => {
		it("should initialize with valid session token", () => {
			handler = new ZentaraHandler(mockOptions)
			expect(handler).toBeInstanceOf(ZentaraHandler)
		...
		it("should not throw error if CloudService is not available", () => {
			...
			expect(() => {
				new ZentaraHandler(mockOptions)
			}).not.toThrow()
			// Constructor should succeed even without CloudService
			const handler = new ZentaraHandler(mockOptions)
			expect(handler).toBeInstanceOf(ZentaraHandler)
		})
		...
		it("should initialize with default model if no model specified", () => {
			handler = new ZentaraHandler({})
			expect(handler).toBeInstanceOf(ZentaraHandler)
			expect(handler.getModel().id).toBe(zentaraDefaultModelId)
		})
	...
	describe("completePrompt", () => {
		...
		it("should handle API errors", async () => {
			...
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				"Zentara Code Cloud completion error: API Error",
			)
		})
	...
	describe("getModel", () => {
		...
		it("should return model info for specified model", () => {
			...
			// xai/grok-code-fast-1 is a valid model in zentaraModels
			expect(modelInfo.info).toBe(zentaraModels["xai/grok-code-fast-1"])
		})

		it("should return default model when no model specified", () => {
			const handlerWithoutModel = new ZentaraHandler({})
			const modelInfo = handlerWithoutModel.getModel()
			expect(modelInfo.id).toBe(zentaraDefaultModelId)
			expect(modelInfo.info).toBeDefined()
			expect(modelInfo.info).toBe(zentaraModels[zentaraDefaultModelId])
		})

		it("should handle unknown model ID with fallback info", () => {
			const handlerWithUnknownModel = new ZentaraHandler({
		...
		it("should return correct model info for all Zentara models", () => {
			// Test each model in zentaraModels
			const modelIds = Object.keys(zentaraModels) as Array<keyof typeof zentaraModels>

			for (const modelId of modelIds) {
				const handlerWithModel = new ZentaraHandler({ apiModelId: modelId })
				const modelInfo = handlerWithModel.getModel()
				expect(modelInfo.id).toBe(modelId)
				expect(modelInfo.info).toBe(zentaraModels[modelId])
			}
		})
	})
	...
	describe("temperature and model configuration", () => {
		...
		it("should use correct API endpoint", () => {
			// The base URL should be set to Zentara's API endpoint
			// We can't directly test the OpenAI client configuration, but we can verify the handler initializes
			handler = new ZentaraHandler(mockOptions)
			expect(handler).toBeInstanceOf(ZentaraHandler)
			// The handler should work with the Zentara API endpoint
		})
	})
