...
export async function testInsertBeforeSymbol(): Promise<TestResult> {
	const tool = "insert_before_symbol"

	try {
		// Create test TypeScript file with functions, classes, and methods
		// Note: NO leading newline to ensure correct line numbers
		const testContent = `// Test file for insert before symbol functionality
...
`
		...
		const testUri = await createTestFile("test-insert-before-symbol.ts", testContent)
		...
		// Test inserting content before 'testFunction' which should be at line 1 (0-based)
		// Position should point to the function name for the go-to-definition to work
		const insertBeforeSymbolParams = {
			textDocument: {
				uri: testUri.toString(),
			},
			position: {
				line: 1, // Line where testFunction is defined (0-based) - second line
				character: 16, // Character position of 'testFunction' after "export function " (0-based)
			},
			content: `// This comment was inserted before testFunction
export function insertedBeforeFunction() {
    return "This function was inserted before testFunction";
}`,
		}
		...
		const block: any = {
			...
			params: {
				...
				_text: JSON.stringify(insertBeforeSymbolParams),
			},
		}
		...
		if (resultData.success === true) {
			...
			const hasTargetFileChanges = changeUris.some(
				(uri) => uri === targetUri || uri.endsWith("test-insert-before-symbol.ts"),
			)
			...
			const targetChanges =
				changes[targetUri] ||
				Object.values(changes).find((_, index) => changeUris[index].endsWith("test-insert-before-symbol.ts"))
			...
			if (
				!insertedContent.includes("insertedBeforeFunction") ||
				!insertedContent.includes("inserted before testFunction")
			) {
				return {
					tool,
					passed: false,
			...
			// Verify that the insertion position is before the target symbol
			// For insert_before_symbol, the range should represent the position before the symbol
			const insertPosition = targetChanges[0].range.start

			// The insertion should happen at or before the line where testFunction is defined
			// Since we're targeting line 1 (0-based), the insertion should be at or before line 1
			if (insertPosition.line > 1) {
				return {
					tool,
					passed: false,
					error: `Insertion position (line ${insertPosition.line}) appears to be after the target symbol rather than before`,
					details: {
						insertPosition,
						expectedMaxLine: 1,
						targetChanges,
			...
			// Success: workspace edit was created successfully and insertion is before the symbol
			return {
				tool,
				passed: true,
				details: {
					message: `Successfully created workspace edit to insert content before symbol`,
