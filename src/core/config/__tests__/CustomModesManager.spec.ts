...
describe("CustomModesManager", () => {
	...
	const mockZentaramodes = path.join(mockWorkspacePath, ".zentaramodes")
	...
	beforeEach(() => {
		...
		;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
			return path === mockSettingsPath || path === mockZentaramodes
		})
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .zentaramodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const zentaramodesModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: settingsModes })
				}
				if (path === mockZentaramodes) {
					return yaml.stringify({ customModes: zentaramodesModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .zentaramodes taking precedence", async () => {
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from zentaramodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .zentaramodes since it takes precedence
		...
		it("should handle missing .zentaramodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .zentaramodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .zentaramodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .zentaramodes precedence", async () => {
			...
			let zentaramodesContent = { customModes: zentaramodesModes }

			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockZentaramodes) {
					return yaml.stringify(zentaramodesContent)
				}
				if (path === mockSettingsPath) {
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockZentaramodes) {
					zentaramodesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .zentaramodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Zentaramodes Mode 1", // .zentaramodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .zentaramodes file when adding project-specific mode", async () => {
			...
			// Mock .zentaramodes to not exist initially
			let zentaramodesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .zentaramodes was created with the project mode
			...
			const writeCall = (fs.writeFile as Mock).mock.calls[0]
			expect(path.normalize(writeCall[0])).toBe(path.normalize(mockZentaramodes))

			// Verify the content written to .zentaramodes
			expect(zentaramodesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				;(fs.readFile as Mock).mockImplementation(async (path: string) => {
					...
					if (path === mockZentaramodes && zentaramodesContent) {
						return yaml.stringify(zentaramodesContent)
					}
					throw new Error("File not found")
				})
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".zentaramodes"),
			...
			it("should import multiple modes at once", async () => {
				...
				expect(result.success).toBe(true)
				expect(zentaramodesContent.customModes).toHaveLength(2)
				expect(zentaramodesContent.customModes[0].slug).toBe("mode1")
				expect(zentaramodesContent.customModes[1].slug).toBe("mode2")
			})
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .zentaramodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .zentara directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".zentaramodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".zentara"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".zentara", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .zentaramodes file", async () => {
			const zentaramodesContent = { customModes: [{ slug: "other-mode", name: "Other Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockZentaramodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockZentaramodes) {
					return yaml.stringify(zentaramodesContent)
		...
		it("should return false when .zentaramodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when rules directory doesn't exist", async () => {
			const zentaramodesContent = { customModes: [{ slug: "test-mode", name: "Test Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockZentaramodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.zentara/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .zentaramodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .zentaramodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .zentaramodes", async () => {
			const zentaramodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .zentaramodes", async () => {
			const zentaramodesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.zentara/rules-code" },
			])
		...
		it("should successfully export global mode with rules from global .zentara directory", async () => {
			// Mock a global mode
			const globalMode = {
