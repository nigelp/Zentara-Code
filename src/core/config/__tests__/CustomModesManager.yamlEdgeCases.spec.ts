...
describe("CustomModesManager - YAML Edge Cases", () => {
	...
	const mockZentaramodes = `${path.sep}mock${path.sep}workspace${path.sep}.zentaramodes`
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithNonBreakingSpaces,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithZeroWidth,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should normalize various quote characters", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithFancyQuotes,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Complex fileRegex handling", () => {
		it("should handle complex fileRegex syntax gracefully", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithComplexFileRegex,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	 ...
	 - slug: "test-mode"
	   ...
	   groups: ["read"]` // Missing closing quote
		...
		it("should provide schema validation error messages", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: invalidSchema,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("UTF-8 encoding", () => {
		it("should handle special characters and emojis", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithEmojis,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle various international characters", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithInternational,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Additional edge cases", () => {
		it("should handle mixed line endings (CRLF vs LF)", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithMixedLineEndings,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle multiple BOMs in sequence", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithMultipleBOMs,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle deeply nested structures with edge case characters", async () => {
			...
			mockFsReadFile({
				[mockZentaramodes]: yamlWithComplexNesting,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
