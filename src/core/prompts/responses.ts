...
**CRITICAL: Use 'fetch_tool_description' if you're uncertain about any tool's format or parameters.**
...
Some advanced tools require a **single JSON object** as the content of the XML tag:

<actual_tool_name>{"parameter1": "value1", "parameter2": "value2"}</actual_tool_name>

**Examples of JSON-in-XML tools:**
- LSP operations: \`lsp_get_document_symbols\`, \`lsp_get_symbol_code_snippet\`, etc.
- Debug operations: \`debug_launch\`, \`debug_set_breakpoint\`, etc.
- Subagent tool: \`subagent\`
- Search operations: \`search_files\`

**How to know which format to use:**
- **Use \`fetch_tool_description\` first** if you're uncertain about a tool's format
