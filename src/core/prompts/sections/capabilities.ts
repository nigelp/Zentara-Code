...
- You can use search_files to perform regex searches across files in a specified directory, outputting context-rich results that include surrounding lines. This tool excels at content-based discovery, finding specific implementations, code patterns, and identifying areas that need refactoring. Use search_files when you know WHAT you're looking for but not WHERE it is - it's particularly powerful for semantic searches, finding function definitions, locating API usage patterns, and discovering code that matches specific criteria.
