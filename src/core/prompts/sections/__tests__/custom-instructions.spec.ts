...
describe("loadRuleFiles", () => {
	...
	it("should filter out cache files from .zentara/rules/ directory", async () => {
		// Simulate .zentara/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.zentara/rules" },
			{ name: ".DS_Store", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.zentara/rules" },
			{ name: "Thumbs.db", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.zentara/rules" },
			{ name: "rule2.md", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.zentara/rules" },
			{ name: "cache.log", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.zentara/rules" },
			{
				name: "backup.bak",
				isFile: () => true,
			...
			{ name: "temp.tmp", isFile: () => true, isSymbolicLink: () => false, parentPath: "/fake/path/.zentara/rules" },
			{
				name: "script.pyc",
				isFile: () => true,
				isSymbolicLink: () => false,
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.zentara/rules/rule1.txt") {
				return Promise.resolve("rule 1 content")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/rule2.md") {
			...
			if (normalizedPath === "/fake/path/.zentara/rules/.DS_Store") {
				return Promise.resolve("DS_STORE_BINARY_CONTENT")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/Thumbs.db") {
				return Promise.resolve("THUMBS_DB_CONTENT")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/backup.bak") {
				return Promise.resolve("BACKUP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/cache.log") {
				return Promise.resolve("LOG_CONTENT")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/temp.tmp") {
				return Promise.resolve("TEMP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/script.pyc") {
		...
		const expectedCacheFiles = [
			"/fake/path/.zentara/rules/.DS_Store",
			"/fake/path/.zentara/rules/Thumbs.db",
			"/fake/path/.zentara/rules/backup.bak",
			"/fake/path/.zentara/rules/cache.log",
			"/fake/path/.zentara/rules/temp.tmp",
			"/fake/path/.zentara/rules/script.pyc",
		]
...
describe("addCustomInstructions", () => {
	...
	it("should include AGENTS.md content along with other rules", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (pathStr.endsWith(".zentararules")) {
				return Promise.resolve("Zentara rules content")
			}
			return Promise.reject({ code: "ENOENT" })
		})
		...
		// Should contain both AGENTS.md and .zentararules content
		expect(result).toContain("# Agent Rules Standard (AGENTS.md):")
		expect(result).toContain("Agent rules content")
		expect(result).toContain("# Rules from .zentararules:")
		expect(result).toContain("Zentara rules content")
	})
...
describe("Rules directory reading", () => {
	...
	it("should return files in alphabetical order by filename", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "zebra.txt", isFile: () => true, parentPath: "/fake/path/.zentara/rules" },
			{ name: "alpha.txt", isFile: () => true, parentPath: "/fake/path/.zentara/rules" },
			{ name: "Beta.txt", isFile: () => true, parentPath: "/fake/path/.zentara/rules" }, // Test case-insensitive sorting
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			const pathStr = filePath.toString()
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.zentara/rules/zebra.txt") {
				return Promise.resolve("zebra content")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/alpha.txt") {
				return Promise.resolve("alpha content")
			}
			if (normalizedPath === "/fake/path/.zentara/rules/Beta.txt") {
		...
		const expectedAlphaPath =
			process.platform === "win32" ? "\\fake\\path\\.zentara\\rules\\alpha.txt" : "/fake/path/.zentara/rules/alpha.txt"
		const expectedBetaPath =
			process.platform === "win32" ? "\\fake\\path\\.zentara\\rules\\Beta.txt" : "/fake/path/.zentara/rules/Beta.txt"
		const expectedZebraPath =
			process.platform === "win32" ? "\\fake\\path\\.zentara\\rules\\zebra.txt" : "/fake/path/.zentara/rules/zebra.txt"
	...
	it("should sort symlinks by their symlink names, not target names", async () => {
		...
		// First call: check if .zentara/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
