...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("zentara-cline")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (zentara-code-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "zentara-code-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("zentara-code-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
